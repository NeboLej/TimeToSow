...
protocol PlantRepositoryProtocol {
    func createRandomPlant(note: Note) async -> Plant
...
//    }
    ...
    //    // Сохранение нового растения + начальной заметки + опционального тега к заметке
        ...
        //            let plant =
                    // 1. Сохраняем растение (seed и pot уже существуют как справочники)
                    ...
                    if try PlantModelGRDB.filter(key: plant.id).fetchCount(db) == 0 {
                        ...
                        if let note = plant.notes.first {
                            var modelNote = NoteModelGRDB(from: note, plantID: plant.id)
...
//                     2. Создаём и сохраняем начальную заметку


        //
        //            try note.insert(db)
                    
    //                print("🌱 New Plant saved with initial Note (tag: \(noteTag?.name ?? "none"))")
                    print("🌱 New Plant saved with initial Note (tag: )")
                }
            } catch {
                fatalError()
            }
